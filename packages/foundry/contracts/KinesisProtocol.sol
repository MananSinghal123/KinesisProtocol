...
error KinesisProtocol__FailedToReceived();
error KinesisProtocol__AmountCantBeZero();
error KinesisProtocol__AddressCantBeZero();
error KinesisProtocol__OnlyOwnerCanCall(address caller, address owner);
error KinesisProtocol__OnlyAllowedAddr(address caller);
error KinesisProtocol__NotEnoughLinkBalance(uint256 currentBalance, uint256 calculatedFees);
error KinesisProtocol__FailedToSendEther();
error KinesisProtocol__CcipReceiveError();
error KinesisProtocol__InsufficientBalance();
error KinesisProtocol__InsufficientShares();

/**
 * @title KinesisProtocol
 * @dev A contract for cross-chain yield farming using AAVE and Chainlink CCIP
 */
contract KinesisProtocol is ERC4626, CCIPReceiver, ReentrancyGuard {
    ...
    modifier onlyOwner() {
        if (msg.sender != owner)
            revert KinesisProtocol__OnlyOwnerCanCall(msg.sender, owner);
        _;
    }

    modifier onlyAllowed(address _caller) {
        if (isAllowed[_caller] != true)
            revert KinesisProtocol__OnlyAllowedAddr(msg.sender);
        _;
    }

    /**
     * @dev Constructor to initialize the KinesisProtocol contract
    ...
    ) ERC4626(_asset) ERC20("KinesisProtocol", "KP") CCIPReceiver(_ccipRouter) {
        owner = _owner;
        i_link = _link;
    ...
    function setOwner(address _owner) external onlyOwner {
        if (_owner == address(0)) revert KinesisProtocol__AddressCantBeZero();
    ...
    function userDeposit(uint256 assets) external nonReentrant returns (uint256) {
        if (assets == 0) revert KinesisProtocol__AmountCantBeZero();
    ...
    function userWithdraw(uint256 shares) external nonReentrant returns (uint256) {
        if (shares == 0) revert KinesisProtocol__AmountCantBeZero();
        if (balanceOf(msg.sender) < shares) revert KinesisProtocol__InsufficientShares();
    ...
    function bridgeAndSupplyAssetToAave(
        ...
        if (fee > IERC20(i_link).balanceOf(address(this)))
            revert KinesisProtocol__NotEnoughLinkBalance(
    ...
    function _ccipReceive(
        ...
        if (!success) revert KinesisProtocol__CcipReceiveError();
    }
